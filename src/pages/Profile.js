...
function Profile() {
  ...
  const removeFavorite = async (type, id) => {
    const endpoint = type === 'meal' ? '/remove_favorite_meal' : '/remove_favorite_workout';
    try {
      ...
      if (response.ok) {
        setProfileData(prevData => {
          const updatedList = prevData[type === 'meal' ? 'favorited_meals' : 'favorited_workouts']
            .filter(item => item !== id);
          return {
            ...prevData,
            [type === 'meal' ? 'favorited_meals' : 'favorited_workouts']: updatedList,
          };
        });

        if (type === 'meal') {
          setMealDetails(prevDetails => prevDetails.filter(meal => meal && meal.id !== id));
        } else {
          setWorkoutDetails(prevDetails => prevDetails.filter(workout => workout && workout.id !== id));
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (profileData.favorited_meals.length > 0) {
      Promise.all(profileData.favorited_meals.map(mealId => fetchMealDetails(mealId)))
  ...
  useEffect(() => {
    ...
    if (profileData.favorited_workouts.length > 0) {
      Promise.all(profileData.favorited_workouts.map(workoutId => fetchWorkoutDetails(workoutId)))
